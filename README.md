...
仪表盘现已包含统一日程、在场提醒和会议录音工作台。日程保留
`source / external_event_id / calendar_id / timezone / updated_at / write_back`
字段；Mac Calendar 同步会包含已同步到 Mac 的手机日历。飞书和企微可通过
`POST /api/assistant/events/import` 导入，默认只读，不会虚假声称已回写。
