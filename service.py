...
from watcherobot.protocol import FLAG_LAST
from watcherobot.recordings import iter_wrec_records, write_pcm_wav
from watcherobot.application import rtc as application_rtc

from assistant import AssistantStore, parse_datetime
...
class AgendaEventRequest(BaseModel):
    source: str = Field(default="local", pattern=r"^(local|macos|phone|feishu|wecom)$")
    external_event_id: str = Field(default="", max_length=240)
    calendar_id: str = Field(default="smart-assistant", max_length=160)
...
class AgendaImportRequest(BaseModel):
    source: str = Field(pattern=r"^(macos|phone|feishu|wecom)$")
    events: list[dict[str, object]] = Field(max_length=500)


class AgendaEventActionRequest(BaseModel):
    action: str = Field(pattern=r"^(done|snooze)$")
    minutes: int = Field(default=10, ge=1, le=1440)


class MeetingStartRequest(BaseModel):
    title: str = Field(default="未命名会议", max_length=160)
    consent_statement: str = Field(
        default="本次会议将录音并用于生成会议纪要。",
        max_length=500,
    )
    max_duration_minutes: int = Field(default=120, ge=1, le=240)


class MeetingMarkRequest(BaseModel):
    label: str = Field(default="重点", max_length=80)
...
class MediaLabService:
    ...
    def __init__(
        ...
        *,
        ...
        self._reminder_worker: threading.Thread | None = None
    ...
    def assistant_status(self) -> dict[str, object]:
        with self._assistant_lock:
            meeting = dict(self._meeting) if self._meeting else None
            if meeting and meeting.get("state") in {"starting", "recording", "stopping"}:
                meeting["elapsed_seconds"] = round(
                    max(0.0, time.monotonic() - self._meeting_started_monotonic), 1
                )
        now = datetime.now(timezone.utc)
        agenda = self._assistant.list_events(
    ...
    def agenda(self, *, hours: int = 48) -> dict[str, object]:
        hours = min(24 * 30, max(1, int(hours)))
        now = datetime.now(timezone.utc)
        return {"events": self._assistant.list_events(now - timedelta(hours=2), now + timedelta(hours=hours))}

    def create_agenda_event(self, payload: Mapping[str, object]) -> dict[str, object]:
        return {"event": self._assistant.upsert_event(payload)}

    def import_agenda_events(self, source: str, events: list[dict[str, object]]) -> dict[str, object]:
        imported = self._assistant.import_events(source, events)
    ...
    def update_agenda_event(self, event_id: str, *, action: str, minutes: int = 10) -> dict[str, object]:
        try:
            event = self._assistant.update_event_state(event_id, action, minutes)
        except KeyError as error:
            raise ValueError("agenda event not found") from error
        return {"event": event}

    def remind_agenda_event_now(self, event_id: str) -> dict[str, object]:
        try:
            event = self._assistant.queue_reminder_now(event_id)
        except KeyError as error:
            raise ValueError("agenda event not found") from error
        with self._assistant_lock:
            meeting_active = bool(
                self._meeting and self._meeting.get("state") in {"starting", "recording", "stopping"}
            )
        if not meeting_active:
            self._start_reminder_worker([event])
        return {"queued": True, "deferred_for_recording": meeting_active, "event": event}

    def _maintain_assistant(self) -> None:
        with self._assistant_lock:
            meeting_active = bool(
                self._meeting and self._meeting.get("state") in {"starting", "recording", "stopping"}
            )
        due = self._assistant.claim_due_reminders(recording_active=meeting_active)
        if due:
            self._start_reminder_worker(due)

    def _start_reminder_worker(self, events: list[dict[str, object]]) -> None:
        with self._assistant_lock:
            if self._reminder_worker and self._reminder_worker.is_alive():
                return
            self._reminder_worker = threading.Thread(
    ...
    def _deliver_reminders(self, events: list[dict[str, object]]) -> None:
        for event in events:
            event_id = str(event.get("id") or "")
            try:
                presence = self._detect_presence() if event.get("presence_check", True) else "present"
                if presence != "present":
                    reason = "sensitive_content_not_spoken" if event.get("sensitive") else f"presence_{presence}"
                    self._assistant.mark_reminder_result(
                        event_id, delivered=False, channel="feishu_or_wecom", reason=reason
                    )
                    continue
                title = str(event.get("title") or "日程")
                self._speak_text(f"提醒：{title}即将开始。")
                self._assistant.mark_reminder_result(event_id, delivered=True, channel="robot_voice")
            except Exception as error:
                self._assistant.mark_reminder_result(
                    event_id, delivered=False, channel="feishu_or_wecom", reason=str(error)
                )

    def _detect_presence(self) -> str:
        if "face_tracking.preview.v1" not in self._robot.capabilities:
            return "unknown"
        started = False
        try:
            self.start_face_tracking(preview=True)
            started = True
            deadline = time.monotonic() + 3.0
            while time.monotonic() < deadline:
                frame = self.face_preview_frame()
                if frame.get("ready") and frame.get("faces"):
                    return "present"
                time.sleep(0.12)
            return "absent"
        except MediaLabBusyError:
            return "unknown"
        finally:
            if started:
                try:
                    ...
                    _LOGGER.exception("Reminder presence preview cleanup failed")

    def _speak_text(self, text: str) -> None:
        safe_text = re.sub(r"\s+", " ", text).strip()[:180]
        if not safe_text:
            return
        speech_dir = self._assistant.root / "speech"
        speech_dir.mkdir(parents=True, exist_ok=True)
        token = str(time.time_ns())
        aiff_path = speech_dir / f"speech-{token}.aiff"
        wav_path = speech_dir / f"speech-{token}.wav"
        try:
            subprocess.run(["/usr/bin/say", "-v", "Tingting", "-o", str(aiff_path), safe_text], check=True, timeout=20)
            subprocess.run(
                ["/usr/bin/afconvert", "-f", "WAVE", "-d", "LEI16@24000", str(aiff_path), str(wav_path)],
                check=True,
                timeout=20,
            )
            with self._operation("assistant_reminder", resources=("microphone", "speaker")):
                playback = self._robot.audio.play_file(wav_path)
                playback.wait(30.0)
        finally:
    ...
    def start_meeting_recording(
        self, *, title: str, consent_statement: str, max_duration_minutes: int
    ) -> dict[str, object]:
        with self._assistant_lock:
            ...
            try:
                recording = self._robot.recordings.start_host("audio", duration=None)
            except Exception:
            ...
            self._meeting_thread = threading.Thread(
                ...
                args=(str(meeting["id"]),),
                daemon=True,
                name="assistant-meeting-recording",
            )
            self._meeting_thread.start()
            return {"meeting": dict(self._meeting)}

    def _meeting_recording_worker(self, meeting_id: str) -> None:
        raw_path = self._assistant.meetings_dir / f"{meeting_id}.wrec"
        ...
        try:
            ...
            with partial.open("wb") as sink:
                while True:
                    now = time.monotonic()
                    should_stop = self._meeting_stop.is_set() or (
            ...
            completed = self._assistant.update_meeting(
                meeting_id,
                state="completed",
            ...
            if recording is not None:
                try:
                    ...
                    pass
            with self._assistant_lock:
                operation = self._meeting_operation
    ...
    def mark_meeting(self, *, label: str, note: str) -> dict[str, object]:
        with self._assistant_lock:
            if not self._meeting or self._meeting.get("state") not in {"recording", "stopping"}:
    ...
    def stop_meeting_recording(self) -> dict[str, object]:
        with self._assistant_lock:
            if not self._meeting or self._meeting.get("state") not in {"starting", "recording", "stopping"}:
                return {"stopping": False, "meeting": dict(self._meeting) if self._meeting else None}
            self._meeting_stop.set()
            self._meeting = self._assistant.update_meeting(str(self._meeting["id"]), state="stopping")
            return {"stopping": True, "meeting": dict(self._meeting)}
    ...
    def assistant_artifact_path(self, filename: str) -> Path | None:
        if re.fullmatch(r"[0-9a-f-]{36}\.(?:wav|wrec)", filename) is None:
            return None
        path = self._assistant.meetings_dir / filename
...
def create_web_app(service: MediaLabService, *, web_root: Path) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI) -> AsyncIterator[None]:
        ...
        try:
            ...
            try:
                await asyncio.to_thread(service.shutdown_assistant)
            except Exception:
                _LOGGER.exception("Assistant meeting shutdown could not be confirmed")
    ...
    async def assistant_status() -> dict[str, object]:
        return await asyncio.to_thread(service.assistant_status)

    @app.get("/api/assistant/agenda")
    async def assistant_agenda(hours: int = 48) -> dict[str, object]:
        return await _run_action(service.agenda, hours=hours)

    @app.post("/api/assistant/events")
    async def create_assistant_event(request: AgendaEventRequest) -> dict[str, object]:
        return await _run_action(service.create_agenda_event, payload=request.model_dump())

    @app.post("/api/assistant/events/import")
    async def import_assistant_events(request: AgendaImportRequest) -> dict[str, object]:
        return await _run_action(
            service.import_agenda_events,
            source=request.source,
            events=request.events,
        )

    @app.post("/api/assistant/calendar/macos/sync")
    ...
    async def update_assistant_event(
        ...
        return await _run_action(
            service.update_agenda_event,
            event_id=event_id,
            action=request.action,
            minutes=request.minutes,
        )

    @app.post("/api/assistant/events/{event_id}/remind-now")
    async def remind_assistant_event(event_id: str) -> dict[str, object]:
        return await _run_action(service.remind_agenda_event_now, event_id=event_id)

    @app.post("/api/assistant/meeting/start")
    async def start_assistant_meeting(request: MeetingStartRequest) -> dict[str, object]:
        return await _run_action(
            service.start_meeting_recording,
            title=request.title,
    ...
    async def stop_assistant_meeting() -> dict[str, object]:
        return await _run_action(service.stop_meeting_recording)
    ...
    @app.get("/assistant-artifacts/{filename}")
    async def assistant_artifact(filename: str) -> FileResponse:
        path = service.assistant_artifact_path(filename)
        if path is None:
